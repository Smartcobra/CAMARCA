...
def _ui_style() -> None:
    st.markdown(
        ...
        .st-key-view_complete_guide_btn button {
            background: linear-gradient(135deg, #0f5b3f 0%, #1d9a6c 100%);
            color: #f3fff9;
            border: 1px solid #7adfb7;
            font-weight: 700;
            border-radius: 10px;
        }
        .st-key-view_complete_guide_btn button:hover {
            border-color: #9df0cd;
            background: linear-gradient(135deg, #11724f 0%, #20ad78 100%);
        }
        </style>
        """,
...
@st.dialog("CAMARCA Complete Guide", width="large")
def _show_complete_guide_dialog() -> None:
    guide_file = Path("docs/camarca_complete_guide.html")
    if not guide_file.exists():
        st.error("Could not find `docs/camarca_complete_guide.html`.")
        return
    html = guide_file.read_text(encoding="utf-8")
    st.caption("Full repository documentation with architecture and module-level explanations.")
...
def _render_evaluation_dashboard() -> None:
    ...
    st.caption("Shows the latest artifacts generated by `advanced_evaluation.py`.")

    if not eval_dir.exists():
        st.info("No evaluation artifacts found yet. Run `uv run python advanced_evaluation.py` first.")
        return
    ...
    with p1:
        if acc_plot.exists():
            st.image(str(acc_plot), caption="Validation/Test Running Accuracy", width="stretch")
        else:
            st.info("`accuracy_curve.png` not found.")
    with p2:
        if loss_plot.exists():
            st.image(str(loss_plot), caption="Validation/Test Running Loss", width="stretch")
        else:
...
def main() -> None:
    ...
    with col2:
        ...
        if st.button("View Animated Architecture", key="view_arch_btn", width="stretch"):
            _show_architecture_dialog()
        if st.button("View Complete CAMARCA Guide", key="view_complete_guide_btn", width="stretch"):
            _show_complete_guide_dialog()

    run_clicked = st.button("Run RCA (SSE Stream)", type="primary", width="stretch")
    ...
    if st.session_state.final_payload is not None:
        ...
        with result_box.container():
            ...
            if st.button("View Raw JSON", key="view_raw_json_btn", width="content"):
